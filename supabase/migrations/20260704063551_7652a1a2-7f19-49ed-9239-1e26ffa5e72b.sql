...
-- Extend profiles with responder capabilities
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS specialization text,
  ADD COLUMN IF NOT EXISTS is_available boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS max_active_cases int NOT NULL DEFAULT 5;

-- Workload helper: open cases per responder
CREATE OR REPLACE FUNCTION public.responder_workload()
RETURNS TABLE (
  user_id uuid,
  first_name text,
  last_name text,
...
AS $$
  SELECT p.user_id, p.first_name, p.last_name, p.email, p.province, p.district,
         p.specialization, p.is_available, p.max_active_cases,
...
-- Auto-assign report: pick best responder by district > province > specialization > workload
CREATE OR REPLACE FUNCTION public.auto_assign_report(_report_id uuid)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF _pick IS NULL THEN
    RAISE EXCEPTION 'No available responder' USING ERRCODE = 'P0001';
  END IF;

  UPDATE public.reports
     SET assigned_to = _pick, status = 'Assigned', updated_at = now()
   WHERE id = _report_id;
